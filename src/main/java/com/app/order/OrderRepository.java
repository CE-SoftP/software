...
public interface OrderRepository extends JpaRepository<OrderDatabase, Integer> {
    ...
    @Query("SELECT i FROM OrderDatabase i WHERE i.customerId = :customerId")
    List<OrderDatabase> findByCustomerId(@Param("customerId")int userId);

    @Query("SELECT i FROM OrderDatabase i WHERE i.confAdmin = :confAdmin")
    List<OrderDatabase> findByConfAdmin(@Param("confAdmin")String confAdmin);

    @Query("SELECT i FROM OrderDatabase i WHERE i.popUpUser = :popUpUser AND i.customerId = :customerId")
